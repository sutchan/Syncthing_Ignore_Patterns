...
| 功能 | 说明 |
|------|------|
| 语言切换 | 右上角下拉框选择 `English` / `中文`，实时切换全部界面文字与日志；选择记忆到 `config.json`；中文文案以 `\u` 转义内嵌，脚本保持纯 ASCII |
| 主题切换 | 右上角下拉框选择 `浅色` / `深色`，即时换肤，选择同样持久化 |
