...
- **版本三轨独立**（docs/project.md §4）：
  - ① Flutter 主轨（当前 **v1.25.2**，CI 单一来源 `VERSION`）= `VERSION` ↔ `app/pubspec.yaml` ↔ `app/lib/state/app_state.dart` `AppState.version` ↔ `app/lib/models/manifest.dart` 示例 ↔ `README*` 徽章/正文。动版本前必 `cat VERSION`+`git log` 实查。
...
- **Dart isolate 闭包捕获陷阱（v1.25.2 事故）**：`Isolate.run(f)` 会序列化 `f` 及其捕获上下文。若 `f` 与另一个捕获了不可发送对象（如 `AppState`/`SettingsStore`/`_Future`）的闭包处于**同一函数作用域**，Dart 共用 context，导致 `f` 也被认为捕获了不可发送对象 → 抛 `Illegal argument in isolate message: object is unsendable`，**真实运行中会中断扫描**。修复：把 `Isolate.run` 闭包抽到**顶层函数**（仅捕获纯参数 String/int），进度回调（`onProgress`，捕获 AppState）只在主 isolate 的 `.then` 里调用。教训：`Isolate.run` 的闭包务必放顶层/独立函数，绝不与其同作用域引用 AppState 的闭包共存。
