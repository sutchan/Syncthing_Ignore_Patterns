...
├── .stignore                 # 标准规则源文件（Apply 依赖，版本 v1.10.0）
...
| 功能 | 说明 |
|------|------|
| 语言切换 | 右上角下拉框 `English` / `中文`，实时切换全部界面与日志文案，选择记忆到 `config.json`（v1.10.0） |
| 主题切换 | 右上角下拉框 `浅色` / `深色`，即时换肤，选择同样持久化（v1.10.0） |
