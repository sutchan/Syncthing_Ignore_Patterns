...
├── app/                      # Dart + Flutter 桌面版（主实现，构建为 exe，v1.20.3）
...
- **主实现（Flutter 桌面版）版本单一来源**：
  - `app/pubspec.yaml` 的 `version:` 字段（如 `1.20.3+1`）
...
- [x] GitHub Actions CI：构建并打包命名归档 `SyncthingIgnoreGUI-v1.20.3-windows-x64.zip`（`.github/workflows/ci.yml`）
...
**Dart + Flutter 桌面版（v1.20.3）为主实现**，构建为独立 `.exe` 分发。两者共享同一
...
- 发布包压缩：CI 打包步骤用 .NET `System.IO.Compression` `CompressionLevel.SmallestSize`（优于 `Compress-Archive` 默认 Deflate），并剔除调试符号 `*.pdb`/`*.exp`/`*.lib`，进一步减小 `.zip` 体积（见 `ci.yml` `Package artifact` 步骤）。
- 预发布版本以 GitHub Release 的 `prerelease` 标记区分，**不在文件名加后缀**。

示例：`SyncthingIgnoreGUI-v1.20.3-windows-x64.zip`
