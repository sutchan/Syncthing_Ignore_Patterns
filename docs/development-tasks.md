...
> 跟踪 Flutter 桌面版（主实现，v1.18.6）相较 PowerShell 遗留版（v1.18.5）的
...
| GitHub Actions CI：构建并打包命名归档 | 🔲 | `SyncthingIgnorePatterns-v1.18.6-windows-x64.zip`（见全局命名约定） |
...
## 版本说明
- Flutter 桌面版：v1.18.6（pubspec `1.18.6+1`，`AppState.version`）
