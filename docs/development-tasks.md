...
> 跟踪 Flutter 桌面版（主实现，v1.20.4）相较 PowerShell 遗留版（v1.18.5）的
...
| UI 冒烟测试（应用壳 + 设置对话框语言切换） | ✅ | `app/test/widget_test.dart` |
...
| GitHub Actions CI：构建并打包命名归档 | ✅ | `SyncthingIgnoreGUI-v1.20.4-windows-x64.zip`（`.github/workflows/ci.yml`） |
...
## 版本说明
- Flutter 桌面版：v1.20.4（pubspec `1.20.4+1`，`AppState.version`）
