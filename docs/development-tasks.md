...
> 跟踪 Flutter 桌面版（主实现，v1.18.10）相较 PowerShell 遗留版（v1.18.5）的
...
| GitHub Actions CI：构建并打包命名归档 | ✅ | `SyncthingIgnoreGUI-v1.18.10-windows-x64.zip`（`.github/workflows/ci.yml`） |
...
## 版本说明
- Flutter 桌面版：v1.18.10（pubspec `1.18.10+1`，`AppState.version`）
